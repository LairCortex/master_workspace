...
| 4 | **W3b** | W | Вертикальная шкала дат: `QListWidget` по дням (пустые дни — строки-заглушки), рейка дат слева со скобками многодневок, липкая дата, chip-фильтр диапазона с live-apply, jump-кнопки к событиям | реализовано, не влито (изменения в рабочем дереве без коммита; «влито» фиксируется отдельным docs-коммитом по факту коммита/мерджа, как `8e8c2e6` для W3); дизайн зафиксирован grill-сессией 2026-08-31 (change `make-event-timeline-vertical-w3b`) |
