...
class TestThemeRoles:
    def test_format_area_is_readonly_mono(self, qtbot):
        from tests.presentation.qml_helpers import find_item

        d = XlsxImportDialog("event")
        qtbot.addWidget(d)
        area = find_item(d.quick, "formatArea")
        assert area.property("readOnly") is True
        assert area.property("mono") is True

    def test_theme_reaches_island_palette(self, qtbot, tmp_path):
