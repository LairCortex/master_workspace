...
Measured on this app (macOS, server 0.4.3, launcher dialog): the window and its buttons come through with their Russian labels («Открыть», «Новая игра», «Импорт», «Светлая тема», «Удалить»), and the three remaining label-less / disabled `Button` entries under `Window` are the native close/minimise/zoom controls, not app widgets (a native app's tree shows the same three). Pressing «Светлая тема» by element id toggled the theme and wrote `~/.nri_manager/ui.json` — since NRI-0016 (D2) both switches, the launcher's (`ThemeCheckBox`) and the menu's, keep the FIXED caption «Светлая тема» and carry the state in the tick only, so the text never means two things; the observed flip to «Тёмная тема» predates that change. The QML game-list rows — absent from the pre-NRI-0012 tree (`query` gave 0 matches) — now carry their full name and a working AXPress (row activation opened the game); the tool labels the node `StaticText`, the `ListItem` role label does not survive the Qt6.10/cocoa projection (follow-up FI-2, see `docs/qa/2026-09-23-accessibility-audit.md`).
...
- **role is the component's** — list rows are `ListItem`, the color swatch `RadioButton`, `MentionField` and the sheet inline editor `EditableText`, mention chips `Link`, openers/icon buttons `Button`; role and press handler live inside the library component (`RowItem`/`ThemeSwatch`/`MentionField`; since NRI-0017 also `ThemeDateField` — `Button` role, press and worst-case width floor all internal — and the press half of `ThemeTabButton`) so a forgetful usage-site cannot silently break the contract.
...
- **description carries the hidden meaning of an activation** — the implemented set is exactly «Открывает карточку», «Открывает событие», «Открыть изображение» (detail panel/image, timeline row), «Открывает упомянутую сущность» (mention chip), «Переходит к сущности» and «Развернуть или свернуть раздел» (world-snapshot rows/headers), «Открывает игру» (launcher game rows) — only where the name doesn't already spell the action; readers read it via `queryAccessibleInterface().text(QAccessible.Description)`/raw `AXDescription`, and the tool not painting it is NOT a defect. Rows get theirs through `RowItem`'s pass-through `accessibleDescription` string property → `Accessible.description` (NRI-0017 F6 closed FI-3: the slot the NRI-0012 map promised now exists and the launcher fills it with «Открывает игру»; the other row usage sites the old map sketched — search/type/sheet-rails — stay role+name+press until a usage-site fills the slot, so the NRI-0017 design map and the code now agree).
...
Three offscreen-only quirks, pinned in `tests/test_qml_stock_controls_accessibility.py`: an unannotated stock Button/CheckBox/TabButton reports an EMPTY `QAccessible.Name` offscreen — text-derived naming exists on the live display only, so the guard pins the offscreen-pinnable half (interface + role + no name-contradiction + no annotation in source) while the "Name equals the visible text, non-empty" half is a live-audit check. An unannotated stock `TabButton` also exposes NO accessibility action at all offscreen (role `PageTab`, empty action list — pinned probe; live B1 was the same hole) — a tab's single activation therefore reaches the tree only through `Accessible.onPressAction`, which since NRI-0017 (F4) lives inside `ThemeTabButton` itself (handler in the library, the role/name face stays штатно, never re-annotated). And with a name set, raw tree reads carry the description — `get_app_state` does not paint it.

What the tree CANNOT address — exactly five fixed limits (the four coordinate limits ①–④ plus one named projection limit ⑤), everything else must be reachable without coordinates (§spec «Пределы координатности зафиксированы»): ① mention-popup rows — no `Accessible.*` on popup delegates, keyboard-only after SetFocus on the field (arrows + Enter); ② typed text of `MentionField` and of the sheet inline editor — the tree offers role and focus but no readable value slot (screenshot for content; the value bridge `QAccessible.installFactory` is follow-up NRI-0013; live, the field's plain text layer does surface its text as its own plain-control name); ③ native date popups and `ThemeComboBox` drop-down menus — separate native windows, their items are not island nodes (the trigger button itself is named); ④ tooltip text via `tooltip_shim.py` — never propagated into the tree; ⑤ the list-item role of library rows — «cocoa-проекция теряет List-роль, press работает»: a `RowItem` row stays addressable by its full name and its `AXPress` performs the row action, but the declared `Accessible.ListItem` role reaches AppKit as static text (raw `System Events` read: `AXStaticText`; audit record `docs/qa/2026-09-25-accessibility-audit.md`). The loss is inside the pinned Qt 6.10 cocoa bridge — a same-build probe showed `Accessible.Cell`/`Row`/`List`/`Button` all survive the projection while only `ListItem` falls through — so no usage-site fix exists, and re-declaring the row role would break the component-owned-role contract above; the offscreen `QAccessible` face stays `ListItem` (pinned). Silently growing this list is a defect: the live `accessibility-audit` diffs the tree against it, and every further limit gets added here in the same change that discovers it (⑤ entered exactly this way, in NRI-0017). Where an element is missing beyond these five, record it in the QA report and file a follow-up rather than faking coordinates. 2026-09-23 live-audit register (`docs/qa/2026-09-23-accessibility-audit.md`), final status after NRI-0017: FI-1 CLOSED — `ThemeDateField` carries role+`onPressAction`+worst-case width floor inside the component; live, the snapshot «Дата» and the dialog «Дата начала»/«Дата конца» project as named `Button`s whose AXPress opens the picker and the caption never loses its year. FI-2 RECLASSIFIED — documented limit ⑤ (addressability and press confirmed live again 2026-09-25). FI-3 CLOSED — `RowItem` gained the pass-through `accessibleDescription` slot and the launcher fills it; raw reads carry «Открывает игру» on live rows. FI-4 CLOSED — the music-edit «✎» button is named at the usage site («Изменить ссылку на музыку»); the live tree carries it. PARTIAL FI-5 — the char-sheets islands (list tabs, preset rows, editor and fill action rows) went live-confirmed 2026-09-25; the remaining menu-invoked islands (doc viewer, xlsx, LLM setup, table host, event types) and `DetailPanel` content stay offscreen-pinned awaiting a real-mouse re-audit. FI-6 (the non-accessibility startup bug) was fixed in NRI-0015 and re-confirmed live in the same run. Open follow-up from the 2026-09-25 final run: OBS-1 (at the saved 1028×708 frame the snapshot header clips «Показать всё» at the panel edge — the worst-case width floor that guarantees the year is a contributor; the spec keeps the year, the row layout on narrow windows gets the follow-up) — `docs/qa/2026-09-25-accessibility-audit.md`.

Window-format norm (вход→формат, change `nri-0014-window-contract-and-docs`) — what a live audit expects from the contract's windows (`window-layout`/`native-focus-repro`): first-run game choice — modal launcher; «Сменить игру…», «Документация», «Changelog», «Настройка LLM…» — non-modal windows with a visible title bar and native close (`NonModal`+`show()`; keys `launcher_switch`/`docs_readme`/`docs_changelog`/`llm_setup` of `presentation/window_registry.MenuWindowRegistry`: a repeated menu entry raises the live window instead of stacking, closing releases the key); «Календарь…» — ApplicationModal top-level window (no nested event loop, the parent MainWindow geometry unchanged); «Импорт из .xlsx…» — sheet whose body carries the working exit button «Отмена»; EventDialog/entity-card — sheets with a `ThemeSheetHeader` line (title + «Закрыть» button = the Esc outcome) and a `color.scrim` dim on the parent sheet when a child sheet is open; standard Qt dialogs and file panels are Russian (`QTranslator` with bundled `qtbase_ru.qm`, pinned `ru` locale; every `QFileDialog` call passes `DontUseNativeDialog`). These windows are addressed without coordinates — native title bar/close come from the OS window, the island controls keep their accessibility contract — so the four coordinate limits above stand untouched; NRI-0014 introduced no new limit.
...
- Global UI prefs: the one `UiPrefsManager` (`infrastructure/ui_prefs/config.py`) owns `~/.nri_manager/ui.json` (0600) — theme name plus `UiPrefs.windows` (window role → saved frame `[x, y, w, h]`). The placement memory of the named windows (`main`, `sheet_list`, `sheet_editor`, `sheet_fill`, `table_host`) lives in `presentation/geometry_memory.py` (`WindowGeometryMemory.restore/attach`: the saved rect is clamped into a connected screen, a first-open role without a placement centers on screen, saves are debounced on move/resize and guaranteed on close). No QSettings and no second prefs file (NRI-0015 T3).
- App journal: «О приложении → Сохранять логи в файл» writes `~/.nri_manager/nri_manager.log` — the same config home as the prefs, never the checkout/exe dir — via a `RotatingFileHandler` (≈2 MB × 3 archives, UTF-8, directory created on demand in `MainWindow._on_log_toggle`; the handler carries no own `setLevel`, so the file promises exactly the root threshold — no unreachable DEBUG; toggling off closes the descriptor). State listeners must unsubscribe explicitly: `ThemeRuntime.add_listener` returns a handle and `remove_listener(handle)` retires it — a window that subscribed (the launcher) unsubscribes on `finished` (NRI-0016 AB5/AB6/D1).
- Table-host session PIN is six digits, regenerated every start (`secrets.randbelow(1_000_000)` formatted `:06d` in `application/services/table_host_service.py`, shown only while the table runs, text selectable) and the bundled web page restates the length twice (`web/index.html` `maxlength`, `web/app.js` length check) — grep-pinned against the service test; the join protocol itself stays a length-agnostic string compare (NRI-0016 TB6).
...
2. **One knowledge, one place.** Entity-type facts (labels, the short tab captions `tab_label` the detail tabs show, plurals, relations, search/LLM flags) live in `domain/entity_registry.py` keyed by `EntityType` (the full `plural_label` stays the tab's accessibility name and tooltip); the sanctioned type↔ORM map is `infrastructure/repositories/__init__.py` and the type↔repository map is `main.py:_build_entity_services` — no parallel string-keyed type dicts elsewhere in `application/domain`. Date-slot/coordinate rules only in `infrastructure/repositories/coord_mapping.py`; cross-cutting constants once: app root `~/.nri_manager` in `infrastructure/paths.py`, image extensions in `domain/allowed_image_extensions.py`, `game_settings` keys in `game_settings_repository.py`/`llm_settings_repository.py`.
...
- Version is NOT single-sourced: four places sync together — `pyproject.toml`, `app/__init__.py::__version__` (the single in-code source the UI reads: the disabled «Версия X.Y.Z» line in the «О приложении» menu, never a literal nor parsed from the CHANGELOG), `CFBundleShortVersionString` in `nri_manager.spec`, and the top section of `docs/CHANGELOG.md`. `tests/test_version_consistency.py` pins `pyproject == __version__` via tomllib; the spec/CHANGELOG halves stay release-time manual syncs (NRI-0016 V7).
